 ...
 * @LastEditTime: 2025-02-12 19:13:16
 */
/*
...
public:  
    ...
    Person(const Person &p)   // p这个人不可被修改，以引用的方式传入
    {
        ...
        // 深拷贝操作       *: 解引用
        p_height = new int(*p.p_height);  // 在堆区开辟一个新空间，内容为要拷贝的数据，这样就有两个空间存了一样的值，不同Person对象的析构函数
                                          // 释放空间时就会释放自己的空间，从而避免了浅拷贝操作时堆区内容重复释放的错误
        cout << "深拷贝构造函数被调用（编译器自动调用）年龄为: " << p_age  << endl;
    }
