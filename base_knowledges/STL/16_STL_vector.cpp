...
vector原理：当插入数据原数组大小不够时，会去内存中根据某中算法找一片新的空间，先将原数组中的数据
...
void test7()
{
    ...
    // 收缩内存方式1：
    vector<int>(v3).swap(v3); // !!!!!    vector<int> (v3) 表示匿名对象(新的)，再按照v3所 用数据个数(这里是10个)来初始化容量和大小，然后与v3交换，交换之后，新的匿名对象及所占内存空间被系统回收
    cout << "收缩内存大小后: " << endl;
